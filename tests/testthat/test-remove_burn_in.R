...
test_that("remove_burn_in: use", {
  ...
  v <- seq(1, 1000)
  w <- remove_burn_in(trace = v, burn_in = 0.0)
  expect_equal(v, w)

  # Remove all a thousand
  v <- seq(1, 1000)
  w <- remove_burn_in(trace = v, burn_in = 1.0)
...
test_that("remove_burn_in: abuse", {
  ...
  expect_error(
    remove_burn_in(trace = v, burn_in = 1.1),
    "remove_burn_in: burn_in must be at most one"
  )

  expect_error(
    remove_burn_in(trace = "not a valid trace", burn_in = 0.1),
    "remove_burn_in: trace must be numeric"
  )
