#' Creates the parameter files in the article, but
#' slimmed down for time measurement
#' @return The filenames of all parameter files created
#' @export
#' @author Richel Bilderbeek
create_parameter_files_timings <- function() {
  filenames <- NULL
  b_index <- 0
  for (b in c(0.3, 0.5)) {
    lambda_index <- 0
        ...
        for (r in c(0.1, 0.01, 0.001)) {
          ...
          for (l in c(1000, 10000)) {
            ...
            save_parameters_to_file(
              ...
              n_alignments = 2,
              sequence_length = l,
              mcmc_chainlength = 10000, # divided by 100
              n_beast_runs = 2,
              filename = filename
            )
            l_index <- l_index + 1
