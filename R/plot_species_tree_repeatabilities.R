plot_species_tree_repeatabilities <- function() {
  ...
  for (trees_filename in list.files(path = ".", pattern = "^(toy_example|example|article)_.*_1_1_1\\.trees")) {
    ...
    nLTT.plot(phy = parameter_file$species_trees_with_outgroup[[2]][[1]],replot = FALSE,lty=1,lwd = 1)
    ...
    get_average_nltt(beast2out.read.trees(trees_filename_5), replot = TRUE, lty = 3, lwd = 1, col = "red")
    get_average_nltt(beast2out.read.trees(trees_filename_6), replot = TRUE, lty = 3, lwd = 1, col = "blue")
    get_average_nltt(beast2out.read.trees(trees_filename_7), replot = TRUE, lty = 3, lwd = 1, col = "green")
    get_average_nltt(beast2out.read.trees(trees_filename_8), replot = TRUE, lty = 3, lwd = 1, col = "purple")
...
plot_species_tree_repeatabilities()
