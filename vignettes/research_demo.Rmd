...
Note that they all have the same random number generator seed. Because
...
for (filename in filenames) {
  ...
  print(length(read_file(filename)$pbd_output$igtree.extant$tip.label))
  nLTT::nLTT.plot(
    read_file(filename)$pbd_output$tree, 
    main = filename
  )
  phytools::plotSimmap(
