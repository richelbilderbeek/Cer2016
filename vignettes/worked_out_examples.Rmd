...
If we want to generate files fresh, set this to `TRUE`:
...
For that, I will create four parameter files:
  ...
  * `worked_out_example_1.RDa`: protractedness is absent
  * `worked_out_example_2.RDa`: protractedness strong
  * `worked_out_example_3.RDa`: protractedness is absent for multiple replicates
  * `worked_out_example_4.RDa`: protractedness is strong for multiple replicates

Here, we create the four parameter filenames:

```{r}
filenames = c(
  "worked_out_example_1.RDa", 
  "worked_out_example_2.RDa", 
  "worked_out_example_3.RDa", 
  "worked_out_example_4.RDa"
)
```
...
```
  ...
  * `worked_out_example_1.RDa`: protractedness is absent, thus speciation completion rate is high, in this case `1.0e6`
  * `worked_out_example_2.RDa`: protractedness strong, thus speciation completion rate is low, in this case `1.0e-1`
  * `worked_out_example_3.RDa`: protractedness is absent for multiple replicates, thus number of sampled species trees, alignments per species trees, and BEAST2 runs are all 2
  * `worked_out_example_4.RDa`: protractedness is strong for multiple replicates,  thus number of sampled species trees, alignments per species trees, and   BEAST2 runs are all 2
...
```{r}
add_species_trees(filename, verbose = FALSE, add_outgroup = TRUE)  
```
...
```{r}
trees_filename <- "worked_out_example_1_1_1_1.trees"
...
```{r}
trees_filename <- "worked_out_example_2_1_1_1.trees"
testit::assert(file.exists(trees_filename))
phylogenies <- rBEAST::beast2out.read.trees(trees_filename)
# To get the densiTree function working, phylogenies must be of class multiphylo
class(phylogenies) <- "multiPhylo"
