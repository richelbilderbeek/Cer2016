...
n_taxa <- length(read_file(filename)$pbd_output$igtree.extant$tip.label)
```

There are `r n_taxa` taxa. All taxa are good species, so the
...
n_taxa <- length(read_file(filename)$species_trees_with_outgroup[[1]][[1]]$tip.label)
```

In the sampled species tree, there are `r n_taxa`.
...
n_taxa <- length(labels(read_file(filename)$alignments[[1]][[1]]))
```

In the alignment, there are `r n_taxa` taxa.
