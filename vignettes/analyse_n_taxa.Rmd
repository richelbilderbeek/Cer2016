...
```{r}
all_trees <- extract_posteriors(file)[[1]][[1]]
plot(all_trees[[1]])
```
...
df <- collect_files_n_taxa(filenames, verbose = TRUE)
...
create_fresh <- TRUE # Set this to TRUE for a >2 mins calculation
if (create_fresh) {
...
For which parameter settings are these?

```{r}
df_big <- merge(x = df_parameters, y = df_n_taxa, by = "filenames", all = TRUE)
