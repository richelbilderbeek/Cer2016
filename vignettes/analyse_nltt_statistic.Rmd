...
nltt_stats_no_na <- nltt_stats[ !is.na(nltt_stats$nltt_stat), ]

nltt_stats_no_na_all_equal <- dplyr::summarise(
  dplyr::group_by(nltt_stats_no_na, filename, sti, ai, pi),
  all_equal = (length(unique(nltt_stat)) == 1)
)
nltt_stats_no_na_all_equal[ nltt_stats_no_na_all_equal$all_equal == TRUE, ]


nltt_stats[ nltt_stats$filename == "article_0_0_2_0_0.RDa", ]


dplyr::summarise(
  dplyr::group_by(df, filename, sti, ai, pi),
...
library(dplyr)
nltt_stats %>% 
  filter(!is.na(nltt_stat)) %>% 
  group_by(filename, sti, ai, pi) %>% 
  summarise(ribir::is_distributed_normally(nltt_stats$nltt_stat))
