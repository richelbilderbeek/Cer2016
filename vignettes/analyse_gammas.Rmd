...
ggplot2::ggplot(
  ...
  ggplot2::xlab("Gamma Statistic") +
  ggplot2::ylab("Count") +
  ggplot2::ggtitle("Posterior Gamma Statistics")
```
...
for (stat in head(df_species_trees$gamma_stat)){
  counter <- counter + 1
  if ( !is.na(stat) && (stat < 2)){
...
counter    <- 0
outliers   <- NULL
for (posterior in head(df_posterior$filenames)){
  counter  <- counter + 1
  counter2 <- 0
