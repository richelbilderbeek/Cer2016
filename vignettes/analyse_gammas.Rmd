...
# Thanks to Robin Lovelace
all_parameter_filenames <- list.files(system.file("extdata", package = "Cer2016"), full.names = TRUE, pattern = "toy_example_.*\\.RDa")
...
csv_filename_parameters    <- find_path("collected_parameters.csv")
  ...
  if (!is.na(stat)){
    ...
    for (filename in row.names(df_parameters)){
      ...
      if (filename == df_species_trees$filenames[counter]) {
        na_sample$filenames <- c(na_sample$filenames, df_species_trees$filenames[counter]) #nolint
...
richel_gammas <- df_comparison[
  df_comparison$filenames %in% rownames(df_parameters_richel),
  ]
head(richel_gammas)
